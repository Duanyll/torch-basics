# From https://huggingface.co/briaai/RMBG-1.4
...
model: Any = None
RMBG_INPUT_SIZE = (1024, 1024)  # (h, w)


def load_rmbg_model(device="cuda"):
    global model
...
@torch.no_grad()
def estimate_foreground(image: torch.Tensor) -> torch.Tensor:
    """
    Estimate foreground mask from image.

    :param image: Input image [C, H, W]
    :return: Foreground alpha mask [H, W]
    """

    c, h, w = image.shape
    image = rearrange(image, "c h w -> 1 c h w")
    image = F.interpolate(image, size=RMBG_INPUT_SIZE, mode="bilinear")
    image = TF.normalize(image, [0.5, 0.5, 0.5], [1.0, 1.0, 1.0])
    prediction = model(image)[0][0]
    prediction = F.interpolate(prediction, size=(h, w), mode="bilinear")
    mn = prediction.min()
    mx = prediction.max()
